...
*   **Bước 4 - Thanh toán & Validation Gate (Bảo vệ tính vẹn toàn):** 
    * Trước khi sinh Link thanh toán PayPal, App Server bắt buộc chạy **Validation Gate** (`HGET` trên Redis) để chặn đứng các User đã hết hạn giữ vé (trả về HTTP 400). Đồng thời, đặt cờ `payment_lock` lên Redis để cấp thời gian ân hạn (Grace Period 60s) cho User đang thao tác.
    * Khi PayPal báo thanh toán thành công, App Server gọi lệnh chốt: `UPDATE seat_inventory SET status='BOOKED' WHERE seatNo=:seat AND reservedBy=:userId`. Nếu thất bại (do hết hạn), tự động chạy **Fallback Update** `UPDATE ... WHERE status='AVAILABLE'`. Nếu vẫn thất bại, từ chối giao dịch và đưa vào diện Hoàn tiền (Refund).
...
*   **Hết hạn giữ ghế (Hold Timeout Rollback tại phân khu SVIP):** Kèm theo hành động giữ ghế thành công, một Delayed Message (10 phút) được đẩy vào RabbitMQ. Nếu sau 10 phút user không thanh toán, Worker tự động chạy và kiểm tra cờ `payment_lock`. Nếu user đang ở trang PayPal (có cờ), Worker sẽ hoãn thu hồi thêm 60 giây. Nếu không có cờ, Worker **BẮT BUỘC thực thi đồng thời 3 lệnh nguyên tử:** 1) Gọi `HDEL` trên Hash hoặc đổi trạng thái Field của ghế đó về `available`. 2) Gọi `DECRBY user:{userId}:concert:{concertId}:zone:svip 1` để hoàn trả lại Quota giới hạn cá nhân. 3) Chạy lệnh `UPDATE seat_inventory SET status='AVAILABLE', reservedBy=NULL` để nhả khóa vật lý dưới PostgreSQL. Cuối cùng kích hoạt Pub/Sub + SSE để chuyển màu ghế sang Xanh cho người khác mua.
